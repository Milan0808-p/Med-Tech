    ...
    <div className="flex min-h-screen bg-gray-100" style={{ backgroundImage: "url('https://media.istockphoto.com/id/1159847028/photo/medical-stethoscope-isolated-on-white-background.jpg?s=612x612&w=0&k=20&c=pWg7zs2AZmeSvdjnWRxlx5Y0wwgnGMpJlOQi065D6mM=')", backgroundSize: 'cover', backgroundPosition: 'center' }}>
      ...
      <div className="flex-grow p-8 ml-64 bg-white bg-opacity-90 shadow-lg rounded-lg m-8">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-4xl font-bold text-center text-gray-900 mb-8" style={{ fontFamily: 'Georgia, serif', textShadow: '1px 1px 2px rgba(0, 0, 0, 0.2)' }}>Patients List</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {patients.length > 0 ? (
              patients.map((patient) => (
                <div
                  key={patient._id}
                  className="bg-white p-6 rounded-lg shadow-lg cursor-pointer hover:bg-blue-100 transition duration-300 transform hover:scale-105"
                ...
                >
                  <h3 className="text-2xl font-semibold text-gray-800 mb-2" style={{ fontFamily: 'Verdana, sans-serif' }}>
                    {patient.fullname.firstname} {patient.fullname.lastname}
                  </h3>
                  <p className="text-gray-600" style={{ fontFamily: 'Arial, sans-serif' }}><strong>Email:</strong> {patient.email}</p>
                  <p className="text-blue-500 mt-2" style={{ fontFamily: 'Tahoma, sans-serif' }}>Click to view progress report</p>
                </div>
              ))
            ) : (
              <p className="text-center col-span-full text-xl text-gray-700" style={{ fontFamily: 'Arial, sans-serif' }}>No patients found.</p>
            )}
          </div>
        </div>
      </div>
